...
StereoFrame::StereoFrame(const Mat &img_l_, const Mat &img_r_, const int &idx_,
                         PinholeStereoCamera *cam_, const double &time_stamp_) : rgbImg_l(img_l_), rgbImg_r(img_r_), frame_idx(idx_), cam(cam_), time_stamp(time_stamp_)
{
...
StereoFrame::StereoFrame(const Mat &img_l_, const Mat &img_r_, const int &idx_,
                         ...
                         const double &time_stamp_) : rgbImg_l(img_l_), rgbImg_r(img_r_), frame_idx(idx_), cam(cam_), time_stamp(time_stamp_)
{
            ...
            if (lr_qdx == rl_tdx && dist_12 <= nn12_dist_th)
            {
                ...
                if (fabsf(points_l[lr_qdx].pt.y - points_r[lr_tdx].pt.y) <= Config::maxDistEpip())
                {
                    ...
                    if (disp_ >= Config::minDisp())
                    {
                        ...
                        //                        PointFeature* point_;
                        Vector2d pl_;
                        pl_ << points_l[lr_qdx].pt.x, points_l[lr_qdx].pt.y;
                        Vector3d P_;
                        P_ = cam->backProjection(pl_(0), pl_(1), disp_);
    ...
    if (Config::hasLines() && !lines_l.empty() && !lines_r.empty())
    {
        ...
        for (int i = 0; i < n_matches; i++)
        {
            ...
            if (lr_qdx == rl_tdx && dist_12 > nn12_dist_th)
            {
                ...
                Vector3d sp_l;
                sp_l << lines_l[lr_qdx].startPointX, lines_l[lr_qdx].startPointY, 1.0;
                Vector3d ep_l;
                ep_l << lines_l[lr_qdx].endPointX, lines_l[lr_qdx].endPointY, 1.0;
                Vector3d le_l;
                le_l << sp_l.cross(ep_l);
                le_l = le_l / sqrt(le_l(0) * le_l(0) + le_l(1) * le_l(1));
                Vector3d sp_r;
                sp_r << lines_r[lr_tdx].startPointX, lines_r[lr_tdx].startPointY, 1.0;
                Vector3d ep_r;
                ep_r << lines_r[lr_tdx].endPointX, lines_r[lr_tdx].endPointY, 1.0;
                Vector3d le_r;
                ...
                // check minimal disparity
                if (disp_s >= Config::minDisp() && disp_e >= Config::minDisp() && fabsf(le_r(0)) > Config::lineHorizTh() && overlap > Config::stereoOverlapTh())
                {
...
void StereoFrame::extractStereoFeatures_ORBSLAM(const int fast_th)
{
    ...
    if (Config::hasPoints() && !(points_l.size() == 0) && !(points_r.size() == 0))
    {
        ...
        for (int i = 0; i < vDistIdx.size(); i++)
        {
            ...
            Vector2d pl_;
            pl_ << points_l[iL].pt.x, points_l[iL].pt.y;
        ...
        for (int i = 0; i < n_matches; i++)
        {
            ...
            if (lr_qdx == rl_tdx && dist_12 > nn12_dist_th)
            {
                ...
                if (disp_s >= Config::minDisp() && disp_e >= Config::minDisp()
                    ...
                    && fabsf(le_l(0)) > Config::lineHorizTh() && overlap > Config::stereoOverlapTh())
                {
                    Vector3d sP_;
                    sP_ = cam->backProjection(sp_l(0), sp_l(1), disp_s);
                    Vector3d eP_;
                ...
                if (fabsf(points_l[lr_qdx].pt.y - points_r[lr_tdx].pt.y) <= Config::maxDistEpip())
                {
                    ...
                    if (disp_ >= Config::minDisp())
                    {
                        ...
                        if (disp_sub_ >= 0)
                        {
                            ...
                            pl_ << points_l[lr_qdx].pt.x, points_l[lr_qdx].pt.y;
                            Vector3d P_;
                            P_ = cam->backProjection(pl_(0), pl_(1), disp_sub_);
                            pdesc_l_.push_back(pdesc_l.row(lr_qdx));
